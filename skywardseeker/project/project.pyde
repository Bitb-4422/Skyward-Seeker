...
class Player:
        ...
        for item in game.power_ups:
            for power_up in item:
                if self.distance(power_up) <= self.r + power_up.r:
                    ...
                    if power_up.collectible == "L":
                        if self.life < 5:
                            self.life += 1
                    if power_up.collectible == "B":      
    ...
    def update_bullets(self):
        ...
        self.bullets[:] = [bullet for bullet in self.bullets if bullet.x > 0 and bullet.x < SCREEN_W]
...
class Button:
    ...
    def display(self):
        ...
        if self.x<=mouseX<=self.x+self.w and self.y<=mouseY<=self.y+self.h:
            fill(self.c2_r, self.c2_g,self.c2_b)
        else:
            fill(self.c1_r,self.c1_g,self.c1_b)
...
class Game:
    ...
    def reset_level2():
        game.player1.life = 5
        game.player1.bullets = []
        game.player2.bullets = []
    ...
    def reset_game_over():
        ...
        game.player1.x = 0
        game.player1.y = GROUND - 100
        game.player2.x = SCREEN_W - 80
        game.player2.y = GROUND - 100
        game.gameover = False
        game.music_L1.pause()
        game.music_L2.pause()
        game.music_L3.pause()
